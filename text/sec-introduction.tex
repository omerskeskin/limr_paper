...
Desirable protocol properties guaranteed by session types include \textit{safety} 
...
safety (\cref{theo-type-safe}), deadlock-freedom \cref{theo-progress} and liveness \cref{theo-sess-live}. the following properties:
\begin{enumerate}
    \item \textbf{Subject Reduction} (\cref{theo-sub-red}): If $\M$ can progress into $\M'$, then $\Gamma$ can progress into $\Gamma'$ such that $\Gamma'$ types $\M'$.
    \item \textbf{Session Fidelity} (\cref{theo-sess-fid}): If $\Gamma$ can progress into $\Gamma'$, 
    then $\M$ can progress into $\M'$ such that 
    $\M'$ is typable by $\Gamma'$.
    
    \item \textbf{Safety} (\cref{theo-type-safe}): If $\M$ can progress into $\M'$ by one or more communications,
    participant $\pp$ in $\M'$ sends to participant $\pq$ and $\pq$ receives from $\pp$,
    then the labels of $\pp$ and $\pq$ cohere.
    \item \textbf{Deadlock-Freedom} (\cref{theo-progress}): Either every participant in $\M$ has terminated,
    or $\M$ can progress.
    \item \textbf{Liveness} (\cref{theo-sess-live}): If participant $\pp$ attemps to communicate with participant $\pq$ in $\M$,
    then $\M$ can progress (in possibly multiple steps) into a session $\M'$ where that communication has occured.   
\end{enumerate}
...
The relevant predicates on these traces, such as fairness, are then defined using linear temporal logic (LTL)\cite{pnueli1977temporal}.
The LTL modalities eventually ($\diamond$) and always ($\square$) can be expressed as least and greatest
fixpoints respectively using expansion laws. This allows us to represent the properties that use these modalities
as inductive and coinductive predicates in Rocq. This approach, together with the proof techniques provided by paco,
