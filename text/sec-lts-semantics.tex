...
\begin{definition}[Transition labels] A transition label $\alpha$ has the following form:
  \begin{align*}
    ...
    & \SEP \lblsync{p}{q}{\ell} && \text{(A synchronized communication from $\pp$ to $\pq$ occurs via message label $\ell$)}\\
  \end{align*}
\end{definition}
...
In Rocq typing context reductions are defined with the predicate \lstin{tctxR} \rocqlink{todo}.
...
\end{align*}
and $\Gamma = \{{\prt{p}}:\T_{\prt{p}}, \; {\prt{q}} : \T_{\prt{q}},\; {\prt{r}}: \T_{\prt{r}}\}$.
