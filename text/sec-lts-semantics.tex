...
In the Coq implementation we implement local typing contexts as finite maps of 
...
Inductive label: Type :=
  ...
  In Coq we represent $\subject{\alpha}$ with the predicate \lstin{ispSubjl p alpha} that holds 
...
In Coq terms, if \lstin{T} is in context \lstin{gamma} then \lstin{wfltt T} holds. This is expressed by the predicate
...
\begin{example}\label{exam:reductions}
  ...
  \T_{\prt{q}} &= \procinmult{\prt{p}}{\ell_0(\tint).\T_{\prt{q}}\;,  \; \ell_1(\tint).\procoutsingle{\prt{r}}{\ell_3(\tint)}{\tend}}\\
...
In Coq, $\Gamma$ is defined the following way:
\begin{tcb}{Coq}
Definition prt_p:=0.
Definition prt_q:=1.
Definition prt_r:=2.
...
Now \cref{eq:red1} can be stated with the following piece of Coq
\begin{tcb}{Coq}
...
\end{array}
\]
In Coq $\lbltrans{\G}{\lblsync{p}{q}{\ell_k}}{\G'}$ is expressed with the coinductively defined (via Paco)
