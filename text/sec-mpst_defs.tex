...
\end{tcb}
\end{definition}
Note that in Rocq we represent the continuations using a \lstin{list} of \lstin{option} types. 
...
\begin{enumerate}
  ...
  are predicates from the Rocq Standard Library \cite[List]{coqstl} that are used to quantify 
...
our approach is explained in \cite{SynchronousSubtyping}, and the Rocq implementation of translation
...
\begin{remark}
  ...
  defined trees in Rocq. Rocq's Leibniz equality is not strong enought to treat 
...
In Rocq we express coinductive relations such as subtyping using the Paco library \cite{paco}.
...
In Rocq these constructions are expressed with the inductive \lstin{isMerge} and the coinductive
\lstin{projectionC}.
\begin{tcb}{Rocq}
...
are captured using the Rocq standard library predicate 
...
$\T \subtp \projfn{\G}{\pr}$. In our Rocq implementation we define the predicate \lstin{issubProj}
...
In Rocq balancedness is expressed with the predicate \lstin{balancedG (G : gtt)}
\end{definition}
...
\begin{definition}[Global Type Tree Context]\label{def:global-ctx}
  ...
  \begin{align*}
   ...
   \SEP \hole_i
  \end{align*}
  In Rocq global type tree contexts are represented by the type \lstin{gtth}
  \begin{tcb}{Rocq}
Inductive gtth: Type :=
  | gtth_hol    : fin -> gtth
