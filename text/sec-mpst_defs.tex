...
We briefly recap the core definitions of local and global type trees, subtyping and projection from
...
is detailed in \cite{srpaper}. From now on we work exclusively on local type trees. Also, as done in \cite{srpaper},
we assume coinductive extensionality and consider isomorphic type trees to be equal.
...
\begin{definition}[Projection] \label[definition]{def:projections}
    ...
    \begin{itemize}
      ...
      that there are $\T_i, i \in I$ such that $\T = \bigsqcap_{i \in I} \T_i$ and $\forall i \in I, \G\proj{r}\T_i $ \hfill {\ruleprojcont}
    \end{itemize}
    where $\bigsqcap$ is the plain merging operator,
    defined  as
    \[
    \T_1 \bigsqcap \T_2 = 
    \begin{cases}
