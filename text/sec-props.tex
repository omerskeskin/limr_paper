...
be referring to local type contexts, global types or sessions, depending on our application.
...
valid.  We use different \lstin{V} based on our application,
for example in the context of local type context reductions \lstin{V gamma gamma'}
...
inductive or coinductive (via Paco) types. 
...
Then fairness can be expressed in LTL with: for all $\pp,\pq$, 
  \begin{align*}
  \square(\mathtt{enabledComm}_{\prt{p},\prt{q},\ell} \implies 
  \lozenge(\mathtt{headComm}_{\prt{p},\prt{q}}))
  \end{align*}
   Similarly, by defining $\mathtt{enabledSend}_{\prt{p},\prt{q},\ell,S}$ that holds iff
...
In Rocq, we implement the LTL operators $\lozenge$ and $\square$ inductively and coinductively (with Paco), 
...
\end{tcb}
\begin{remark}
Note that the LTS of local type contexts has the property that, once a transition between participants 
$\pp$ and $\pq$ is enabled, it stays enabled until a transition between $\pp$ and $\pq$ occurs. This makes
\lstin{fair_path} equivalent to the standard formulas \cite[Definition 5.25]{baier} for strong fairness 
($\square \lozenge \mathtt{enabledComm}_{\pp,\pq} \implies \square \lozenge \mathtt{headComm_{\pp,\pq}}$) and 
weak fairness ($ \lozenge \square \mathtt{enabledComm}_{\pp,\pq} \implies \square \lozenge \mathtt{headComm_{\pp,\pq}}$).
\end{remark}
...
First, we prove that the typing context obtained by direct projections of \lstin{g},
...
The proof that \lstin{gamma_proj} is live proceeds by well-founded induction on the tree height \cite{clrs} of 
the grafting (\cref{lem-grafting}) of the global type \lstin{g}.
...
We prove the following helper lemmas:
\begin{itemize}
  \item The height of the \lstin{p}-grafting of \lstin{g} is not smaller than the \lstin{q}-grafting \rocqlink{todo}.
  \item If the \lstin{p}-grafting and \lstin{q}-grafting of a global type \lstin{g'} 
  have the same height, then any fair path beginning with the direct projection context of \lstin{g'} 
  eventually contains a $\lblsync{p}{q}{\ell}$ transition \rocqlink{todo}.
  \item The height of the \lstin{p}-grafting of \lstin{g} strictly decreases 
  with every transition involving \lstin{q} \rocqlink{todo},
  and doesn't increase with the transitions not involving \lstin{q} \rocqlink{todo}.
\end{itemize}
These lemmas followed by well-founded induction on the height of the \lstin{p}-grafting of the 
global type the head of \lstin{xs} is projected from gives the desired transition. 

In the second step of the proof we extend association on to paths to get, for each local type context reduction path 
\lstin{xs} that begins with \lstin{gamma}, another local reduction path \lstin{ys} beginning with \lstin{gamma_proj}
...
\end{corollary}
\begin{proof}
  Let \lstin{xs} be the fair local reduction path starting with \lstin{gamma}.
