...
\end{definition}
\begin{tcolorbox}[colback=blue!10]
Informally, liveness says that every communication request on the path is eventually answered.
With our fairness assumption \cite{fairness}, we focus on "sensible" reduction paths
where every communication that's enabled by both the participants is eventually executed.
