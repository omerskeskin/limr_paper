...
\end{tcb}  
\todo{this section is wrong, fix it}Informally, we say that the global type context $\mathbb{G'}$ is a \textit{proper prefix} of 
$\mathbb{G}$ if any path to a leaf in $\mathbb{G'}$ is a proper prefix of a path in 
$\mathbb{G}$. 
...
height \cite{clrs} of a global type tree context.
\begin{tcb}{Coq}
...
\begin{definition}[Path Association]
