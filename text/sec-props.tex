...
%From now on, we assume all reduction paths we consider in the Rocq implementation
%satisfy this validity condition. Further, when talking about local type context reduction paths,
%we assume that all the transition labels involved in the path are synchronous transitions i.e.
%they are of form $\lblsync{p}{q}{\ell}$.

%We can now define fairness and liveness on local type context reduction paths. 
...
\lstin{alwaysCG} \rocqlink{todo}. The Rocq definition of \cref{def-fair-live} amounts to the following \rocqlink{todo}:
\begin{tcb}{Rocq}
