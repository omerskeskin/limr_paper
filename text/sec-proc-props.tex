...
typable session can (\cref{lem-fair-scheduling}), and this will be enough to prove our desired liveness property.
\end{remark}
...
[Path Typing]\label{def-typ-path}
...
\begin{enumerate}[(i)]
  \item The empty path is typed with the empty path.
...
\end{lemma}
\begin{proof}
We can construct a fair path starting from \lstin{M} with the following algorithm:  
\end{proof}
\begin{theorem}[Liveness by Typing]
For a session \lstin{Mp}, if \lstin{exists gamma, typ_sess Mp gamma} then \lstin{live_sess Mp}.
\end{theorem}
\begin{proof}
We detail the proof for the send case of \cref{def-live-sess}, the case for the receive is similar.
Suppose that \lstin{betaRtc Mp M} and \lstin{unfoldP M ( (p <-- p_send q ell e P') \|\|\| M')}.
Our goal is to show that there exists a \lstin{M''} such that
\lstin{betaRtc M ((p <-- P')\|\|\|M'')}. 
First, observe that it suffices to show that 
\lstin{betaRtc ((p <-- p_send q ell e P') \|\|\| M') M''}
for some \lstin{M''}. Also note that \lstin{typ_sess M gamma} for some \lstin{gamma}
by \cref{theo-sub-red}, therefore 
\lstin{typ_sess ((p <-- p_send q ell e P') \|\|\| M') gamma}
by \cref{lem-typ-unfold}.
Now let \lstin{xs} be the fair reduction path starting from \lstin{((p <-- p_send q ell e P') \|\|\| M')}, 
...
We have \lstin{betaRtc ((p <-- p_send q ell e P') \|\|\| M'') M_0}
by \lstin{M_0} being on a reduction path starting from \lstin{M}. We also have that 
