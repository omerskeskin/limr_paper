...
We have two kinds of typing judgements and type contexts.
$\Theta_\T, \Theta_\kf{e} \vdashp \PT : \T$ says that the single process $\PT$
can be typed with local type $\T$ using expression and type variables from $\Theta_\T, \Theta_\kf{e}$.
On the other hand, $\Gamma \vdashm \M$ expresses that session $\M$ can be typed by the local type context (\cref{def-type-ctx})
...
\cref{tbl:proc} states the standard \cite{srpaper,SynchronousSubtyping} typing rules for processes, 
...
We can now prove some properties typed sessions. The following theorems relating
session reductions to types underlie our results.
...
\cref{lem-typ-unfold} says that typing is preserved after unfolding. 
\cref{theo-sub-red} shows that the typing context reduces along with the session it types.
\cref{theo-sess-fid} is an analogue of \cref{theo-sub-red} in the opposite direction.
\end{tcolorbox}
...
\begin{theorem}[Communication Safety \rocqlink{todo}]\label{theo-type-safe}
...
\end{theorem} 
\begin{tcolorbox}[colback=blue!10]
\cref{theo-type-safe} means that typed sessions evolve to sessions where if participant \lstin{p}
wants to send to \lstin{q} with label $\ell$, and \lstin{q} is listening to receive from \lstin{p},
then \lstin{q} is able to receive with label $\ell$.
\end{tcolorbox}
...
\end{theorem}
\begin{tcolorbox}[colback=blue!10]
\cref{theo-progress} says that the only way a typed session has no reductions available is if 
it has terminated.
\end{tcolorbox}
...
which has the following fairness property: whenever a transition with label $\lblsync{p}{q}{\ell}$ is enabled,
  a transition with label $\lblsync{p}{q}{\ell'}$ eventually occurs for some $\ell'$.
It can be shown that such a path always exists, and that path can be constructed using the axioms
\lstin{constructive_indefinite_description} and \lstin{excluded_middle_informative} \rocqlink{todo}.

Now by extending \cref{theo-sub-red} onto paths, let \lstin{ys} be a
